...
- 资料来源与手动刷新步骤见 `docs/library/README.md`。当前为本地发布候选，尚未确认正式域名状态。

## 2026-09-23 About NOW 更新获正式站同步授权

- L-One 已明确要求将六工具、透明背景、NOW 区布局及滚动更新同步至 `l-one.asia`，任务编号 `A-20260923-04`。功能候选包含 `A-20260923-02`、`A-20260923-03` 的已审内容。
- 发布前去除了下排作品条残留的固定浅灰底，使其显示连续的 NOW 页面底材。GitHub 合并、EdgeOne 自动部署及正式域名验收均待本轮执行并回读，不预先标记为成功。

## 2026-09-23 About v4.2 发布已获确认

- `A-20260920-01` 的 About v4.2 候选已为 4:3 时间线接入更明显的冷灰白底材、保守放大和两侧/底部渐隐；可由 CSS 控制的矩形窗口和 UI 统一为 `2px` 内的直角过渡。用户提供纹理文件的 SHA-256 为 `A500F68DABD9FB3754533069270877CD4B40F2D7ACC6BAC19E0E6574C492408E`。
- 视频内容、人物、时间线文字、页面结构、作品集入口和远端配置不变；动态取样不可用时安全回退为静态白灰渐隐。
- L-One 已于 2026-09-23 明确确认部署 About。本次发布将同时包含其唯一必要的本地跳转目标 `portfolio.html`，不改动 DNS、EdgeOne 配置、服务器数据或任何生产凭据。
- 发布仍须依次完成精确提交、PR、受保护分支合并与正式域名回读；完成前不得将生产状态标记为已验证。

## 2026-09-23 About v4.2 NOW 区本地续改

- `A-20260923-02` 仅在本地候选分支 `work/A-20260923-02-about-now-material-marquee` 完成 NOW 区四个工具标识的透明背景版本：L-1 Text、Capture、StoryFlow、拓印。新 PNG 均为 `1254×1254` RGBA，Alpha 范围 `0–255`；原始图片保持不改。
- 下排作品滚动卡片由 `225px` 调整为 `259px`（+15%），卡片间距仍为 `16px`，动画周期同步由 `90s` 调整为 `103.5s`，以维持原有像素滚动速度。悬停/焦点卡片放大 `8%`，同列其他卡片按两侧等量位移维持视觉间距。
- “查看过往作品”键帽移到主观点下、下排作品滚动条上；其上下间距使用同一变量。此轮没有提交、推送、合并或部署，正式域名状态没有新增验证。

## 2026-09-23 About v4.2 NOW 六工具与纵向压缩本地续改

- `A-20260923-03` 仅在本地候选将 NOW 工具改为六种、各重复一次的无缝列表：既有 L-1 Text、Capture、StoryFlow、拓印，加 Wave、One BAR。Wave 采用用户提供的透明 RGBA 原图；One BAR 新增透明 PNG 副本；既有图不覆盖。
- 保持工具与下排作品卡片尺寸，收紧 NOW 的章节头/主观点/入口留白，使默认桌面视口能更早露出下排作品。下排作品条使用纯净页面底色，移除灰色纹理。
- 上排周期由 `90s` 调为 `128.571s`，下排由 `103.5s` 调为 `147.857s`，两条滚动速度均为此前的 70%。下排至下一段留白由 `28px` 增至 `56px`；左上品牌标记在向下滚动超过 `20px` 时淡出，右上菜单保持可用。
- 本轮仍为本地候选，未提交、推送、合并或部署。
...
- `A-20260902-01` 已通过 PR [#10](https://github.com/L-One-Tools/L-One-main-site/pull/10) 合并至 `main`；
  合并提交 `3d570089591d1fee9da75f746e59de1d9756bc98`。EdgeOne 已更新正式 Store 与详情页。
- 下载端点已独立复核为 HTTP 200、响应文件名 `L-1-.-v0.2.2-.zip`、`16,700` bytes、
  SHA-256 `FB9441ED595E24E6A6B9E8DD3D994E353B412FA22EFC44C923AD7E7D499DF055`；Release、
  版本资料和反馈 Issue 均为 HTTP 200。`https://l-one.asia/store/`、详情页和公开 Catalog 均为 HTTP 200，
  线上文件字节哈希与合并仓库一致。
- L-One 本轮提供的 2D 标识仅用于当前 Store/首屏本地预览；3D 标识保留为候选、未进入页面。
  真实 L-One 页面测试原图仅获本地预览授权，未来公开提交前必须重新确认标识授权、替换为获批
  清理素材，并重新完成三端审核。
- 2026-09-02 复审版移除自行添加的黄/红功能卡片和章节注解；首段改为任务弹窗的无工具栏裁切，
  下载目录结果合并进“网页自己滚动”，并增加 Chrome 解压加载的文字提示。仍仅为本地预览。
- `l_one_decision`：L-One 于 2026-09-03 通过此最新调整版，批准基线是当前分支的未提交页面状态，
  不是更早预览版。正文、图片、主布局、版本、Logo、人工分行、功能承诺和交互均已锁定；变更即需重新审核。
- 下载与反馈字段已上线；正式详情页桌面 1440×900、平板 834×1112、手机 390×844 无横向溢出，
  下载与反馈均指向已核验地址。回滚使用 `git revert -m 1 3d570089591d1fee9da75f746e59de1d9756bc98`。

更新时间：2026-08-31

## 2026-08-31 L-1 File To Text 2.0.8 自有下载域名迁移已发布

- `A-20260831-01` 已将官网下载按钮、稳定 Catalog 和离线备用 Catalog 切换到
...
- 本地 Catalog、回退、全站和 Motion Library 审计通过；PR #8 合并后，正式详情页已检测到
  两个自有下载按钮。浏览器实际下载流程由运营继续完成三端复核。
